...
namespace iobuffer
{
	...
	public:
		...
		virtual int Read(std::istream &) = 0; // read a buffer from the stream
		virtual int Write(std::ostream &) const = 0; // write a buffer to the stream

		// these are the direct access read and write operations
		virtual int DRead(std::istream &, int recref); // read specified record
		virtual int DWrite(std::ostream &, int recref) const; // write specified record
